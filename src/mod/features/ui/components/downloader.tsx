...
import { Info, FolderOpen, Folder, Download, ImageDown } from "lucide-react";
...
export function Downloader() {
  ...
  // pageType/collectionId are derived from window.location on every render.
  // The downloader relies on the frequent re-renders (e.g. the react-query
  // player-meta poll) to pick up SPA navigation, so we intentionally do NOT
  // memoize them with stale deps.
  ...
  async function downloadTracks(trackIds: string[], quality: QualityEnum) {
    ...
    for (let i = 0; i < tracks.length; i++) {
      ...
      if (track!.available === false) {
        console.log("[Downloader] Track is not available", track);
      ...
      if (downloadInfo.isErr()) {
        console.log("[Downloader] DownloadUrl not available", track);
      ...
      Sentry.captureMessage("tracks_downloaded", { level: "info" });

      console.log("[Downloader] got track download url", track, downloadInfo.value);
  ...
  const handleDownloadCover = async () => {
    const meta = getTrackMetaQuery.data;
    if (!meta) {
      toast.warning("Нет данных о треке");
      return;
    }
    const coverUri = meta.coverUri || meta.albums?.[0]?.coverUri;
    if (!coverUri) {
      toast.error("У трека нет обложки");
      return;
    }
    const fileName = `${meta.artists?.map((a: any) => a.name).join(", ") || "cover"} - ${meta.title || "track"}`;
    const result = await window.yandexMusicMod.downloadCover(coverUri, fileName);
    if (result?.ok) {
      toast.success("Обложка сохранена", { description: result.path });
    } else {
      toast.error("Не удалось сохранить обложку", { description: result?.error });
    }
  };
    ...
    <ExpandableCard title="Скачать треки" icon={<Download className="h-4 w-4" />} opened={true}>
      <div className="flex flex-col gap-5 pt-2 px-3">
        ...
        <Button
          ...
          disabled={!getTrackMetaQuery.isSuccess || !getTrackMetaQuery.data}
          onClick={handleDownloadCover}
        >
          <ImageDown className="h-4 w-4 mr-1" /> Обложка
        </Button>
